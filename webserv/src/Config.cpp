...
void ConfigFile::handleEvents() {
    ...
    for (size_t i = 0; i < clients_to_disconnect.size(); ++i) {
        ...
        std::cout << "[Timeout] Client " << fd << " disconnected due to inactivity" << std::endl;
        std::cout << "size of clients_to_disconnect: " << clients_to_disconnect.size() << std::endl;
        // _clients.erase(fd); // Remove from _clients map
        clients_to_disconnect.erase(clients_to_disconnect.begin() + i);
        // std::string timeoutResp = "HTTP/1.1 408 Request Timeout\r\nConnection: close\r\n\r\n";
        ...
        // No need to erase from _clients here if cleanupDisconnectedClient already does that
    }
    // struct timeval tv;
    // gettimeofday(&tv, NULL);
    ...
    //     if (now - client.getLastActivityTime() > 60000) { // 60 seconds timeout
    //         std::cout << "[Timeout] Client " << fd << " disconnected due to inactivity" << std::endl;
    //         cleanupDisconnectedClient(fd); // You already have this function
    //         it = _clients.erase(it);       // Remove from map safely
    //     } else {
    //         ++it;
    //     }
    // }
