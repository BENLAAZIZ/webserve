...
void	pars_location(std::vector<std::string>& tokens, Location &loc)
{
	...
	if (tokens[0] == "location")
	{
		...
		throw::std::runtime_error("Error structer of block location" );
	else if (tokens[0] == "root")
	{
		if (!loc.root.empty())
				throw::std::runtime_error("dublicated directive " + tokens[0] + " in location block");
	...
	else if (tokens[0] == "autoindex" && loc.auto_index_set)
	{
		throw::std::runtime_error("nembre of argment for " + tokens[0] + " directive");
	}
	else if (tokens[0] == "autoindex")
    {
        ...
        if (tokens[1] != "on" && tokens[1] != "off")
            throw::std::runtime_error("invalid argument for auto_index");
        if (tokens[1] == "on")
            loc.autoindex = 1;
    }
	...
	else if (tokens[0] == "cgi_extension")
	{
		...
		for (size_t i = 1; i < tokens.size(); i++)
		{
			...
			throw::std::runtime_error("unkowng directive " + tokens[0] + " for location block");
	begin_loc = 0;
}
	...
	for (size_t i = 2; i < serv.size() - 1 ; i++)
	{
		...
		if (tokens[0] == "listen")
        {
            ...
            for (size_t i = 1; i < tokens.size(); i++)
            {
                ...
                if (nbr < 1024 || nbr > 65535)
                    throw::std::runtime_error("invalid port number " + tokens[i]);
    	...
    	else if (tokens[0] == "host")
		{
			if (!server.host.empty())
				throw::std::runtime_error("dublicated directive " + tokens[0]);
			check_number_argment(tokens, 2); 
		...
		else if (tokens[0] == "server_name")
		{
			if (!server.server_name.empty())
				throw::std::runtime_error("dublicated directive " + tokens[0]);
			check_number_argment(tokens, 2);
			if (tokens[1] == "default_server")
				server.is_default = true;
			server.server_name = tokens[1];
		}
		else if (tokens[0] == "client_max_body_size" && clt_max_bdsize_set)
		{
			throw::std::runtime_error("dublicated directive " + tokens[0]);
		}
        else if (tokens[0] == "client_max_body_size" && !clt_max_bdsize_set)
		{
			clt_max_bdsize_set = true;
			check_number_argment(tokens, 2);
		...
		else if (tokens[0] == "error_page" )
        {
            ...
            std::vector<std::string> &errors = server.error_pages[nbr];
            for(size_t i = 2; i < tokens.size(); i++)
                errors.push_back(tokens[i]);
            server.error_pages[nbr] = errors;
        }
		else if (tokens[0] == "root")
		{
