...
ssize_t Response::send_header_response(size_t CHUNK_SIZE, std::string path, Request &request, int flag_autoindex, const std::string& header_from_param) 
{
	(void)CHUNK_SIZE;
	...
	headers << "Date: " << get_current_http_date() << "\r\n";
	if (!request.isCGI)
	...
	std::cout << "Connection: " << request.keepAlive << std::endl;
	// pause();
...
void Response::handleGetResponse(int *flag, Request &request, int flag_delete, Server_holder &serv_hldr) {
    ...
    *flag = 0;
	...
	if (request.isCGI && flag_delete == 0)
	{
		...
		if (send_header_response(CHUNK_SIZE, path, request, 1, headers_cgi) == 1)
		{
