...
Location* find_location(const std::string& path, const std::vector<Location>& locations) {
    ...
    for (std::vector<Location>::const_iterator it = locations.begin(); it != locations.end(); ++it) {
        ...
        // Match if path starts with location path
...
void getHeaderBody(Request& request, Server_holder& serv_hldr) {
    ...
    //=======================================
    // find location /upload
    (void)serv_hldr;
    // std::cout << "+++++++++ find location /upload" << std::endl;
    // Location* loc = find_location("/upload", serv_hldr.locations);
    // if (loc == NULL) {
    //     std::cerr << "Location not found" << std::endl;
    //     request.set_status_code(404);
    //      request.endOfRequest = 1;
    //     return;
    // }
