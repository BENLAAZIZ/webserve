...
// 		// std::cout << "pid: " << pid << std::endl;
// 		// std::cout << "status: " << st0atus << std::endl;
// 			if (res_pid == pid && !WEXITSTATUS(status))
// 			{
// 				char	buffer[1000];
// 				size_t	beytes_read;
// 				while ((beytes_read = read(pipe_fd[0], buffer, sizeof(buffer))))	
...
// 				break;
// 			}
// 			else if (!res_pid && (time(NULL) - start >= 10))
// 			{
// 				kill(pid, SIGKILL);
// 				time_out = true;
// 				break;
			...
			if (res_pid == pid) { 
				if (WIFEXITED(status)) {
					...
					if (exit_status == 0) {
						...
						// ✅ Error inside CGI script (Python or PHP)
						char buffer[1000];
						size_t bytes_read;
						while ((bytes_read = read(pipe_fd[0], buffer, sizeof(buffer))))	
							result.append(buffer, bytes_read);

						// Optional: log the error message in result
						close(pipe_fd[0]);
						return 500;
					}
					break;
