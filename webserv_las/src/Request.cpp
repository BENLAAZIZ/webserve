...
void Request::handleOtherTypes(Request& request) {
	while(1) {
		if (request._requestBuffer.empty())
			break;
		if (request.chunked.fd == -1)
		...
		write(request.chunked.fd, request._requestBuffer.c_str(), request._requestBuffer.size());
		chunked.bytesRemaining += request._requestBuffer.size();
		request._requestBuffer.clear();

		if (chunked.bytesRemaining >= request.getContentLength()) {
			request.endOfRequest = true;
			close(request.chunked.fd);
			request.chunked.fd = -1;
			break;
		}
	}
}
