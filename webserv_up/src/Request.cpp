...
void	Request::reset()
{
	...
	formData.name.clear();
    formData.filename.clear();
    formData.contentType.clear();
    formData.data.clear();
	formData.fd = -1;
	formData.flagData = 0;
	formData.pos_new_line = 0;
	formData.dataCrlf.clear();
	formData.flag = 0;
	formData.header.clear();
	formData.pos = 0; 
	formData.totalBytesWritten = 0;
	formData.totalBytesRead = 0;
	formData.totalBytes = 0;
...
void Request::handleChunkedData(Request& request) {
    ...
    while (1) {
        ...
        if (request.chunked.chunkSize == -1)
            openFile(request.getHeaders().at("Content-Type"), request.chunked, request.my_root);
		
		if (request.chunked.fd == -1) {
...
void Request::handleBoundary(Request& request, Server_holder& serv_hldr) {
	...
	while (1) {
        ...
        if (!request.formData.flag && request.formData.pos != std::string::npos)
		{
			...
			std::cout << "66666666666 endOfRequest: " << request.endOfRequest << std::endl;
			getHeaderBody(request, serv_hldr);

		}
		// if (request.endOfRequest)
		// {
		// 	std::cout << "///////////////////// endOfRequest: " << request.endOfRequest << std::endl;
		// 	break;
		// }

		if (request.formData.filename.empty()) {
...
void Request::handleChunkedBoundary(Request& request, Server_holder &serv_hldr) {
	while (true) {
        if (request._requestBuffer.empty())
            break;
	...
	handleBoundary(request, serv_hldr);
}
