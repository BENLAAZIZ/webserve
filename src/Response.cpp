		...
		if (request.hasHeader("Range")) {
			...
			if (pos != std::string::npos) {
				...
				if (dash != std::string::npos) {
					...
					start = static_cast<size_t>(std::strtoul(startStr.c_str(), NULL, 10));
					if (!endStr.empty())
						end = static_cast<size_t>(std::strtoul(endStr.c_str(), NULL, 10));
					if (end >= file_size)
						end = file_size - 1;
...
std::string generateAutoIndex(const std::string& dirPath, const std::string& uriPath = "/") {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (!uriPath.empty() && uriPath[uriPath.length() - 1] != '/')
            html << "/";
        html << name;
        if (isDir) html << "/";
...
void Response::handleGetResponse(int *flag, Request &request, int flag_delete, Server_holder &serv_hldr) {
    ...
    *flag = 0;
    ...
    if (this->is_file) {
		...
		ssize_t bytesSent = send(_clientFd, html.c_str(), html.length(), 0); 
		if (bytesSent < 0) {
			if (errno == EAGAIN || errno == EWOULDBLOCK)
			{
				*flag = 0;
...
std::string	Response::get_code_error_path(int errorCode) const {
	...
	switch (errorCode) {
		...
		case 415: code_path = "/415.html"; break;
...
std::string	Response::get_status_message(int errorCode) const
{
	...
	switch (errorCode) {
		...
		case 415: errorMessage = "415 Unsupported Media Type"; break;
